...
# $1 = Original $0 (i.e. script name)
{
   ...
   baseurlfallback="http://vault.centos.org/$oldcentver/os/x86_64/Packages"
...
# Get everything setup and do a few basic checks
{
   ...
   then
      ...
      if [ "$arch" = "i686" ]
      then
         warning "32-bit support is deprecated and will be removed in a future release"
      fi
...
# Extract and install libraries from CentOS 7 RPMs
{
   ...
   *)
   ...
   install_library  "libstdc++-4.8.5-4.el7"       1611014806      318064    "$relusrlib/libstdc++.so.6"  "$baseurl"             0
   install_library  "libstdc++-4.8.3-9.el7"       103833770       314828    "$relusrlib/libstdc++.so.6"  "$baseurlfallback"     1
   ;;
   esac
}
